...
type PuntoRow = {
  ...
  product: {
    name: string; sku: string; category_id: string | null; unit_label: string;
    is_active: boolean; dias_entrega: number | null; dia_pedido: string | null;
  } | null;
};

type FilaReposicion = {
  productId: string; sucursalId: string; sucursalNombre: string;
  nombre: string; sku: string; categoryId: string | null; unit: string;
  stockActual: number; minimo: number | null; pedido: number | null; maximo: number | null;
  diasEntrega: number | null; cantidadSugerida: number | null;
};
...
export default async function ReposicionPage({
  ...
  searchParams: Promise<{ sucursal?: string; categoria?: string; q?: string }>;
}) {
  ...
  const [{ data: sucursales }, { data: categories }, { data: puntosRaw }, { data: stockRaw }] = await Promise.all([
    ...
    (admin as any)
      ...
      .select(`
        ...
        product:products(name, sku, category_id, unit_label, is_active, dias_entrega, dia_pedido)
  ...
  for (const p of puntosRaw ?? []) {
    ...
    filas.push({
      ...
      stockActual,
      minimo:      p.punto_minimo,
      pedido:      p.punto_pedido,
      maximo:      p.punto_maximo,
  ...
  const diaPedidoMap = new Map((puntosRaw ?? []).filter((p) => p.product?.dia_pedido).map((p) => [p.product_id, p.product!.dia_pedido]));

  const pasaFiltros = (f: FilaReposicion) => {
  ...
  const bajoPedido = filas
    .filter((f) => f.pedido != null && f.stockActual <= f.pedido)
    .filter(pasaFiltros)
    .sort((a, b) => a.sucursalNombre.localeCompare(b.sucursalNombre) || a.nombre.localeCompare(b.nombre));

  const pedidoHoy = filas
    .filter((f) => diaPedidoMap.get(f.productId) === hoy)
    .filter((f) => f.maximo == null || f.stockActual < f.maximo)
    .filter(pasaFiltros)
    .sort((a, b) => a.sucursalNombre.localeCompare(b.sucursalNombre) || a.nombre.localeCompare(b.nombre));

  function Tabla({ filas, vacio }: { filas: FilaReposicion[]; vacio: string }) {
          ...
          <tbody className="divide-y divide-neutral-100">
            {filas.map((f) => (
              <tr key={`${f.sucursalId}:${f.productId}`} className="hover:bg-neutral-50/80 transition-colors">
                ...
                <td className="px-4 py-3 text-right text-neutral-400 tabular-nums">{f.minimo ?? "—"}</td>
                <td className="px-4 py-3 text-right text-neutral-400 tabular-nums">{f.pedido ?? "—"}</td>
                <td className="px-4 py-3 text-right text-neutral-400 tabular-nums">{f.maximo ?? "—"}</td>
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1200px]">
      ...
      {pedidoHoy.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-neutral-800 mb-1">
            Pedido semanal de hoy — {DIA_LABEL[hoy]}
          </h2>
          <p className="text-xs text-neutral-400 mb-3">
            Productos con día fijo de pedido que coincide con hoy -- revisalos aunque el stock todavía esté bien, si no se piden hoy se pierde el ciclo hasta la próxima semana.
          </p>
      ...
      <p className="text-xs text-neutral-400 mt-4">
        La cantidad sugerida es un punto de partida (máximo − stock actual) para armar el pedido a mano -- todavía no genera un pedido real al proveedor.
      </p>
    </div>
  );
}
